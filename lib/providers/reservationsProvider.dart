...
class Reservations with ChangeNotifier {
  ...
  //-----------------------------coiffeurDetails--------------------------------
  Future fetchCoiffeurDetails(int id) async {
...
//    };
  ...
  //-----------------------------make favourite --------------------------------
  Future<FavModel> makeFav(int id) async {
  ...
  Future<void> fetchDays(int id) async {
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        notifyListeners();
        return _dayHourModel..days = dayHourModel.days.reversed.toList();
      } else {
